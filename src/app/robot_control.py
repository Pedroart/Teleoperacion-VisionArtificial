...
with open(f"/home/pedroart/dev_ws/src/Teleoperacion-VisionArtificial/data/scalpel_positions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", mode="w", newline="") as csvfile:
    csv_writer = csv.writer(csvfile)
    csv_writer.writerow(["timestamp", "x", "y", "z"])  # Cabecera del archivo CSV
    ...
    while not rospy.is_shutdown():
        ...
        timestamp = time.time()
        csv_writer.writerow([timestamp,end_effector_position[0], end_effector_position[1], end_effector_position[2]])
