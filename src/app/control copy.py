...
def joint_control_callback(msg):
    ...
    Q0 = msg.data
     # Normalizamos cada valor de Q0 al rango [-2π, 2π]
    Q0_normalized = [(q % (2 * np.pi)) if q >= 0 else (q % (-2 * np.pi)) for q in Q0]
    
    # Ajustamos para que esté en el rango [-2π, 2π]
    Q0_normalized = [q if q <= 2 * np.pi else q - 2 * np.pi for q in Q0_normalized]
    Q0_normalized = [q if q >= -2 * np.pi else q + 2 * np.pi for q in Q0_normalized]
