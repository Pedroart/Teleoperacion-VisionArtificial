...
def calculate_pose_error(T_actual, T_desired):
    ...
    # Convertir matrices de rotación a cuaterniones
    quat_actual = R.from_matrix(R_actual).as_quat()
    quat_desired = R.from_matrix(R_desired).as_quat()
    
    # Calcular el error de rotación como un vector de ángulo-eje usando cuaterniones
    rotation_error = R.from_quat(quat_desired) * R.from_quat(quat_actual).inv()
    rotation_vector = rotation_error.as_rotvec()
    
    # Combinar errores de posición y rotación en un solo vector
