...
def calculo_posiciones(results):
    if results.pose_world_landmarks is not None:
        # Joints for wrist, elbow, shoulder
        joints = [mp_pose.PoseLandmark.RIGHT_WRIST, mp_pose.PoseLandmark.RIGHT_ELBOW]

        # Put world landmark positions for the joints into numpy array
        world_landmarks = np.array(
        ...
        wrist_position = world_landmarks[0]  # Coordinates for the right wrist
        elbow_position = world_landmarks[1]  # Coordinates for the right elbow
...
with mp_holistic.Holistic(
    ...
    while cvcam.is_opened():
        ...
        # Process results and publish positions
        if results.pose_world_landmarks:
            wrist_position, elbow_position = calculo_posiciones(results)
            print(wrist_position)
            
            if wrist_position is not None and elbow_position is not None:
                # Prepare and publish wrist position
                wrist_msg = Float64MultiArray()
                wrist_msg.data = wrist_position
                wrist_pub.publish(wrist_msg)

                # Prepare and publish elbow position
                elbow_msg = Float64MultiArray()
                elbow_msg.data = elbow_position
